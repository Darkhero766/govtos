...
function localChat(messages: { role: 'user' | 'assistant'; content: string }[]): { reply: string; journey: Journey } {
  const value = messages.filter(m => m.role === 'user').at(-1)?.content?.trim() || '';
...
export async function POST(request: Request) {
  ...
  try {
    if (operation === 'chat') {
      const messages = Array.isArray(body.messages) ? body.messages.slice(-12).map((m: { role?: string; content?: unknown }) => ({ role: m.role === 'assistant' ? 'assistant' as const : 'user' as const, content: safeText(m.content) })) : [];
      ...
      try { const result = parseJson<{ reply: string; journey: Journey }>(response.output_text); if (!['urgent', 'standard', 'status'].includes(result.journey)) result.journey = 'standard'; return NextResponse.json(result); } catch { return NextResponse.json(localChat(messages)); }
    }
    if (operation === 'triage') {
      ...
      const result = parseJson<{ journey: Journey; reason: string }>(response.output_text); if (!['urgent', 'standard', 'status'].includes(result.journey)) throw new Error('Invalid AI journey'); return NextResponse.json(result);
    }
    if (operation === 'draft') {
