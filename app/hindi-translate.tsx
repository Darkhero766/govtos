...
export default function HindiTranslate() {
  useEffect(() => {
    ...
    const attrOriginals = new WeakMap<Element, { placeholder?: string; title?: string }>();
      ...
      try {
        ...
        nodes.forEach(text => {
          ...
          if (hindi) {
            ...
            if (translated) text.nodeValue = translated;
          } else {
        ...
        document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input[placeholder], textarea[placeholder]').forEach(el => {
          if (!attrOriginals.has(el)) attrOriginals.set(el, { placeholder: el.placeholder });
          const original = attrOriginals.get(el)?.placeholder ?? '';
          if (hindi) el.placeholder = translateText(original) ?? original;
          else el.placeholder = original;
        });
